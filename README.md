## Welcome to InGame Mod Configs!
[Compiling InGame Mod Configs](#compile-ingame-mod-configs) - for those that want the latest unreleased features
...
[Setup InGame Mod Configs](#setup-ingame-mod-configs)

[Compile InGame Mod Configs](#compile-ingame-mod-configs)

[Updating Your Repository](#updating-your-repository)

#### Setup Java
The Java JDK is used to compile InGame Mod Configs.
...
#### Setup Git
Git is used to clone InGame Mod Configs and update your local copy.
...
#### Setup InGame Mod Configs
This section assumes that you're using the command-line version of Git.

1. Open up your command line.
2. Navigate to a place where you want to download InGame Mod Configs's source (eg `C:\Development\Github\Minecraft\`) by executing `cd [folder location]`. This location is known as `mcdev` from now on.
3. Execute `git clone git@github.com:Lunatrius/InGame-Mod-Configs.git`. This will download InGame Mod Configs's source into `mcdev`.
4. Right now, you should have a directory that looks something like:

***
    mcdev
    \-InGameModConfigs
        \-InGameModConfigs's files (should have build.gradle)
***

#### Compile InGame Mod Configs
1. Execute `gradlew setupDevWorkspace`. This sets up Forge and downloads the necessary libraries to build InGame Mod Configs. This might take some time, be patient.
    * You will generally only have to do this once until the Forge version in `build.properties` changes.
2. Execute `gradlew build`. If you did everything right, `BUILD SUCCESSFUL` will be displayed after it finishes. This should be relatively quick.
    * If you see `BUILD FAILED`, check the error output (it should be right around `BUILD FAILED`), fix everything (if possible), and try again.
3. Go to `mcdev\InGameModConfigs\build\libs`.
    * You should see a `.jar` file named `InGameModConfigs-#.#.#-#.#.#.#-universal.jar`.
